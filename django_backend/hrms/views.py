...
class RegisterAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Map role correctly
        if "ADMIN" in role:
            mapped_role = User.Role.ADMIN
        elif "HR" in role:
            mapped_role = User.Role.HR
        else:
            mapped_role = User.Role.EMPLOYEE
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        if not (request.user.is_superuser or request.user.is_staff or request.user.role in {"ADMIN", "HR"}):
            return Response({"detail": "You do not have permission to create employees."}, status=status.HTTP_403_FORBIDDEN)
        return super().create(request, *args, **kwargs)

    def update(self, request, *args, **kwargs):
        if not (request.user.is_superuser or request.user.is_staff or request.user.role in {"ADMIN", "HR"}):
            return Response({"detail": "You do not have permission to update employees."}, status=status.HTTP_403_FORBIDDEN)
        return super().update(request, *args, **kwargs)

    def destroy(self, request, *args, **kwargs):
        if not (request.user.is_superuser or request.user.is_staff or request.user.role in {"ADMIN", "HR"}):
            return Response({"detail": "You do not have permission to delete employees."}, status=status.HTTP_403_FORBIDDEN)
