...
if (res.status === 401) {
  ...
  const fetchAttendance = async (date = attendanceDate) => {
    const data = await fetchJson(`${API}/attendance?date=${date}`);
    setAttendanceLogs(data.data || []);
  };

  const fetchDashboard = async (date = attendanceDate) => {
  ...
  const fetchMonthlyReport = async (month = reportMonth) => {
        ...
        <div className="relative w-full max-w-[420px] app-fade-in">
          ...
          <div className="rounded-[28px] border border-white/10 bg-slate-950/50 p-8 shadow-2xl shadow-indigo-950/50 backdrop-blur-2xl">
            ...
            <button
              ...
              className="btn-primary mt-2 w-full rounded-xl py-3.5 text-sm font-extrabold tracking-wide"
            >
              {authMode === "login" ? "Access Dashboard" : "Create Administrator"}
            </button>
      ...
      <aside className="relative z-20 hidden min-h-screen w-[280px] flex-col p-4 lg:flex">
        <div className="flex min-h-[calc(100vh-2rem)] flex-col rounded-[24px] border border-white/10 bg-slate-950/70 p-5 shadow-2xl shadow-indigo-950/40 backdrop-blur-2xl">
          ...
          <nav className="mt-6 flex-1 space-y-1">
            {navItems.map(([page, Icon, label]) => (
              <button
                ...
                className={`nav-item group flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-semibold ${
                  activePage === page
                    ? "nav-item-active text-white"
                    : "text-slate-400 hover:bg-white/[0.06] hover:text-white"
                }`}
              >
                <Icon size={18} className={activePage === page ? "text-cyan-300" : "text-slate-500 group-hover:text-indigo-300"} />
                {label}
              </button>
            ))}
          </nav>
          ...
          <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-transparent p-4">
            ...
            <button onClick={logout} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-rose-400/25 bg-rose-500/10 px-4 py-2.5 text-sm font-bold text-rose-300 transition hover:bg-rose-500/20">
              <LogOut size={16} />
              Sign out
            </button>
          </div>
        ...
        {activePage === "dashboard" && (
          <>
            ...
            <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
              <StatCard icon={Users} label="Total Employees" value={dashboard.totalEmployees} accent="indigo" />
              <StatCard icon={UserCheck} label="Present Today" value={dashboard.present} accent="emerald" />
              <StatCard icon={UserX} label="Absent" value={dashboard.absent} accent="rose" />
              <StatCard icon={Clock3} label="Late Arrivals" value={dashboard.late} accent="amber" />
              <StatCard icon={CalendarDays} label="On Leave" value={dashboard.leaves} accent="violet" />
            </div>
            <DashboardOverview
              chartData={chartData}
